...
const QuantityPicker = (props) => {
    ...
    switch (nm) {
      ...
      case "input": {
        if (props.min === undefined && props.max === undefined) {
          ...
          event.target.value >= Number(props.min) &&
          event.target.value <= Number(props.max)
        ) {
          if (!isNaN(event.target.value)) {
            setValue(event.target.value);
            props.onChange && props.onChange(event.target.value);
  ...
  return props.smooth ? (
    <div
      style={{ width: props.width ? props.width : "10rem" }}
      data-quantity=""
    >
      <button
    ...
    <span className="quantity-picker">
      ...
      <input
        // style={{ width: props.width ? props.width : "4rem" }}
      ...
      <button
        className={`${
          props.disableInc ? "mod-disable " : "" }quantity-modifier modifier-right`}
        onClick={handleChange("increment")}
      >
        &#xff0b;
      </button>
